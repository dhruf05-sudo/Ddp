(() => {
  ...
  function isLocallyHidden(postId) {
    return hiddenPosts().includes(String(postId));
  }
  ...
  async function api(path, options = {}) {
    ...
    try {
      ...
      // Keep useful error if server returns non-JSON.
    }
  ...
  function applyAppearance() {
    const value =
      localStorage.getItem(KEYS.appearance) || 'system';
  ...
  function setAppearance(value) {
    ...
    $$('[data-appearance]').forEach(input => {
      input.checked =
        input.dataset.appearance === value;
    });
  }
  ...
  function filterPosts(posts, query) {
    ...
    return posts
      ...
      .filter(post =>
        !q ||
        `${post.topic || ''} ${post.content || ''} ${post.authorName || ''}`
          .toLowerCase()
          .includes(q)
      )
      .sort(
  ...
  async function loadPosts(query = '') {
    try {
      ...
      // IMPORTANT:
      // Apply the local hidden-post filter to server posts too.
      return filterPosts(posts, query);

    } catch (error) {
      if (localPosts().length) {
        return filterPosts(localPosts(), query);
      }
        ...
        <div class="pollOptions">
          ...
          ${options.map((option, index) => {
            ...
            return `
              <button
                type="button"
                class="pollOption ${chosen ? 'chosen' : ''}"
                data-vote="${escapeHTML(post.id)}"
                data-option="${index}"
                ${disabled ? 'disabled' : ''}
              >
  ...
  function postCard(post) {
    ...
    const reacted =
      post.reacted ??
      (post.reactions || []).includes(me.id);

    const saved =
      post.saved ??
      (post.savedBy || []).includes(me.id);
    ...
    return `
      ...
      >
        ...
        <div class="postActions">
          ...
          <button
            type="button"
            data-toggle-comments="${escapeHTML(post.id)}"
            aria-label="Comments"
          >
            <svg xmlns="http://www.w3.org/2000/svg"
              width="24"
  ...
  function renderError(container, error) {
    ...
    container.innerHTML =
      `<div class="emptyState">${escapeHTML(message)}</div>`;
  }
  ...
  function bindFeed(container) {
    ...
    $$('[data-react]', container).forEach(button => {
      ...
      button.onclick = async () => {
        ...
        try {
          await api(
            `/posts/${encodeURIComponent(button.dataset.react)}/react`,
            {
              method: 'POST'
            }
          );
    ...
    $$('[data-save]', container).forEach(button => {
      ...
      button.onclick = async () => {
        ...
        try {
          await api(
            `/posts/${encodeURIComponent(button.dataset.save)}/save`,
            {
              method: 'POST'
            }
          );
    ...
    // COMMENTS
    $$('[data-toggle-comments]', container)
      .forEach(button => {

        button.onclick = () => {

          const target = $(
            `[data-comments="${CSS.escape(button.dataset.toggleComments)}"]`,
            container
          );
      ...
      .forEach(form => {
        ...
        form.onsubmit = async event => {
          ...
          try {
            ...
            await api(
              `/posts/${encodeURIComponent(form.dataset.commentForm)}/comments`,
              {
                method: 'POST',
                body: JSON.stringify({
                  text: commentText
            ...
            $(
              `[data-comments="${CSS.escape(form.dataset.commentForm)}"]`,
              container
      ...
      .forEach(button => {
        ...
        button.onclick = async () => {
          ...
          try {
            ...
            await api(
              `/posts/${encodeURIComponent(button.dataset.vote)}/vote`,
              {
                method: 'POST',
                body: JSON.stringify({
                  option:
        ...
        button.onclick = async () => {
          ...
          if (selected === 'hide') {
            ...
            try {
              ...
              // Tell server the request was received.
              await api(
                `/posts/${encodeURIComponent(postId)}/hide`,
                {
                  method: 'POST'
                }
              );
